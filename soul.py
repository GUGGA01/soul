...
@bot.message_handler(commands=['fuckl'])
def handle_soul(message):
    user_id = str(message.chat.id)
    if user_id in allowed_user_ids:
        if user_id not in admin_id:
            
            if user_id in soul_cooldown and (datetime.datetime.now() - soul_cooldown[user_id]).seconds < 30:
                response = "You Are On Cooldown . Please Wait 30sec Before Running The /fuck Command Again."
                bot.reply_to(message, response)
                return
        ...
        if len(command) == 4:  
            ...
            if time > 240:
                ...
                record_command_logs(user_id, '/soul_compiled', ip, port, time)
            ...
            response = "✅A͢v͢a͢i͢l͢a͢b͢l͢e͢ r͢i͢g͢h͢t͢ n͢o͢w͢✅ :- /fuck <king> <soulking> <time>"  
    else:
        response = " ミ🥹★ 𝘈𝘤𝘤𝘦𝘴𝘴 𝘭𝘦 𝘭𝘦 𝘣𝘳𝘰 ★🥹彡DM - @iNSANE_010 ."
...
def welcome_start(message):
    ...
    response = f'''ıllıllı⭐🌟 W͙e͙l͙c͙o͙m͙e͙ t͙o͙ CUMBACK BOT 😈 🌟⭐ıllıllı \n {user_name}! \n🄱🄶🄼🄸 🄺🄸 🄶🄰🄽🄳 🄼🄰🅁🄽🄴 🄰🄰 🄶🅈🄴😜
